...
namespace TAS.Overlays
{
    public class MinesLadder : IOverlay
    {
        ...
        {
            return new string[] { string.Format("{0}: display ladder spawn info", Name) };
        }
            ...
            if (Game1.currentLocation is MineShaft mine)
            {
                    ...
                    (last_characterCount != mine.EnemyCount))
                {
                    ...
                    foreach (KeyValuePair<Vector2, Object> current in Game1.currentLocation.Objects.Pairs)
                    {
                        ...
                        {
                            rockCounters.Add(current.Key, LadderInfo.StonesRemaining(mine, current.Key));
                        }
                    }
                    // check if a ladder exists
                    hasLadder = LadderInfo.HasLadder(out Vector2 loc);
                    if (hasLadder)
                        ladderLocation = loc;
                    else
                        ladderLocation = Vector2.Zero;
